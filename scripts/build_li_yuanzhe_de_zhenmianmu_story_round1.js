...
const BOOK = "李远哲的真面目";
const SLUG = "li_yuanzhe_de_zhenmianmu";
const ROUND = "story_round1";
const STATUS = "校对轮";
const ID_PREFIX = "LYZDZMM";
...
function buildAggregateRows() {
  ...
  return orderedSlugs.flatMap((slug) =>
    readRowsFromCsv(path.join(booksRoot, slug, `${ROUND}.csv`)).map((row) => normalizeAggregateRow(row, slug))
  );
}
...
function candidateScore(paragraph, found) {
  ...
  if (/故事|典故|掌故|我讲|讲一个|举一个|举例|例子|有一个|有一次|有一天|笑话/u.test(paragraph)) score += 6;
  if (/他说|她说|问|答|拒绝|逃|杀|死|跪|叩头|罚跪|抓|骗|哭|笑/u.test(paragraph)) score += 3;
  if ((paragraph.match(/[“”]/gu) || []).length >= 4) score += 2;
  if (/主旨|收件者|寄件者|报导|全文如下|年谱|专档|公文|函|附件|判决|决定书|新闻稿|记者会|目录|第\d+条/u.test(paragraph)) score -= 2;
...
function main() {
  ...
  const manifest = {
    ...
    proofreadDrops: [],
    proofreadTightened: ["鲍林被学生推荐续做研究"],
    totalChars: validation.totalChars,
    aggregateCount: aggregate.rows.length,
    aggregateBooks: aggregate.books,
    criteria:
      "只收李敖正文中转述、引用并用来说明道理的、可独立复述的小故事；排除他人附录、案情材料、行政文件、证据链、政治判断和总库已有重复故事。",
    excludedByStandard,
    proofreadNotes: [
      ...
      "收短鲍林被学生推荐续做研究一条，从“鲍林1954年得诺贝尔化学奖”起，保留故事核心和原文措辞。",
      "吴稚晖假关说信总库已有；浦大邦故事出自附录他人文章；司机命案、危楼、黑官、旷职等为案情或行政材料，均排除。"
    ],
