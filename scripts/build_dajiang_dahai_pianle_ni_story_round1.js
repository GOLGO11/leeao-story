...
function writeNotes(rows, validation, aggregate, manifest) {
  ...
  const lines = [
    ...
    `- 提取轮：${manifest.originalExtractionCount} 条`,
    `- 校对删除：${manifest.proofreadDropCount} 条`,
    `- 校对增补：${manifest.proofreadAddCount} 条`,
    `- 切口收紧：${manifest.proofreadTightenedCount} 条`,
    `- 校对后：${validation.count} 条`,
    ...
    `- 提取轮保留 ${manifest.originalExtractionCount} 条；校对轮未删除、未增补，仍保留 ${validation.count} 条。`,
    `- 校对轮收紧 ${manifest.proofreadTightenedCount} 条正文切口：删去导语、尾评、政论延伸和故事外说明，只保留故事主体。`,
...
function main() {
  ...
  const manifest = {
    ...
    criteria:
      "只收李敖正文中讲成可独立复述、带人物行动或问答转折、并用来说明道理的小故事；排除龙应台口述史材料、战争经过、史料考证、新闻事件链、书目说明、纯语录、薄事实和总库已有重复故事。",
    excludedByStandard,
    duplicateChecks: [],
    extractionNotes: [],
    proofreadNotes: [
