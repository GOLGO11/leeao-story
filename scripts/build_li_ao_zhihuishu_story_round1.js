...
const BOOK = "李敖智慧书";
const SLUG = "li_ao_zhihuishu";
const ROUND = "story_round1";
const STATUS = "校对轮";
const ID_PREFIX = "LAZHS";
...
const selections = [
  ...
  },
  {
    ...
    paragraph: 11,
    title: "医生保证开肺炎药就死于肺炎",
    start: "又有一事，一病人向某医师抱怨说",
    end: "（When I prescribe for pneumonia, you die of pneumonia.）"
  }
];
...
function main() {
  ...
  const manifest = {
    ...
    criteria:
      "校对轮逐条复核叙事闭合度、来源限定、原文连续性与总库重复，并用异体译名反查全书遗漏；只保留可脱离政治评论、新闻事件链和李敖自身论战独立复述，有明确场景、人物行动、转折或结果，并用于揭示人物或说明道理的紧凑故事；排除李敖自己的经历、薄事实、纯引文、资料清单、被正文证伪的传闻和总库已有同核故事。",
    excludedByStandard,
    duplicateChecks: [
      "校对后2则正文已对总库做规范化全文包含与人物关键短语检索，未发现同文或同核故事。",
      "校对增加阿里斯太提斯、阿里斯太狄斯、阿里斯泰迪斯等异体译名检索，定位LATS007与LAYM013并删除提取轮重复条目。",
      "《寂寞小阳春》、秦舞阳、朝三暮四、蝙蝠寓言、刘邦陆贾、张作霖、梁实秋与吴稚晖均已定位总库既有ID并排除。",
    ...
    proofreadDrops: [
      {
        id: "LAZHS001",
        title: "阿里斯太提斯替文盲写下放逐自己的名字",
        reason: "以异体译名反查后确认与总库LATS007、LAYM013为同核故事，校对删除。"
      }
    ],
    proofreadTightened: [],
    ...
    proofreadNotes: [
      "提取轮3则中保留2则、删除1则；无新增、正文边界调整或标题调整。",
      "删除阿里斯太提斯故事后，将原LAZHS002与LAZHS003压紧为LAZHS001与LAZHS002，确保编号连续、非空且全库唯一。",
      "035章远洋小岛六十天才来一次游轮只有背景设定与观点，没有人物行动、转折和结果，不补作故事。",
      "其余明确标作故事、寓言或笑话的候选均为总库同核故事、新闻错误清单、政治事件或李敖自身经历。"
    ],
    extractionNotes: [
      `候选扫描覆盖 ${sourceFiles().length} 个正文文件，机器候选 ${candidateCount()} 条。`,
      `提取轮保留 ${ORIGINAL_EXTRACTION_COUNT} 条，正文均能回到连续原文。`,
      "全部156个机器候选均已浏览，其中33个五分以上、19个六分以上候选重点回看上下文。",
      "入选集中在古代掌故与带明确反转的讽刺故事。",
      "李敖自己的经历、政治案件与新闻事件链、被正文证伪的传闻、语录清单和薄例子不按故事收入。",
