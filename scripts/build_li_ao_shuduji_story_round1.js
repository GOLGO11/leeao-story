...
const BOOK = "李敖书牍集";
const SLUG = "li_ao_shuduji";
const ROUND = "story_round1";
const ID_PREFIX = "LASDJ";
const STATUS = "校对轮";
const OUT_DIR = path.join(ROOT, "data", "books", SLUG);
const NOTES_PATH = path.join(ROOT, "notes", "li_ao_shuduji_story_round1.md");
const CANDIDATE_SCAN = "notes/li_ao_shuduji_candidate_scan.tsv";
...
const selections = [
  ...
  },
  {
    ...
    paragraph: 8,
    title: "蔡元培只送到校长室门口",
    start: "第一任中央研究院院长蔡元培先生",
    end: "岂不值得今天的中央研究院院长想想吗？"
  },
  {
    prefix: "004",
    paragraph: 7,
...
function findSourceRoot() {
  ...
  const bookDir = fs
    ...
    .find((name) => name.startsWith("007.") && name.includes(BOOK));
...
function main() {
  ...
  const manifest = {
    ...
    criteria:
      "书信类提取轮只收文中被李敖讲出来、可独立复述、并用来说明观点的小故事、笑话、历史掌故、文学掌故或外部轶事；排除李敖自己的官司、出版、人际往来、坐牢经历、材料链、人物履历、单句引语和纯评论。",
    excludedByStandard: [
      "李敖自己的官司、出版、人际往来、坐牢经历和政治论战材料原则上不收。",
      "案情链、人物履历、现实政治材料、单句典故、单句引语和纯评论材料不收。",
      "总表已有同质或更完整版本的故事不重复收入。"
    ],
    extractionNotes: [
      `候选扫描覆盖 ${sourceFiles().length} 个正文文件，通用候选 ${candidateCount()} 条。`,
      "提取轮保留 12 条，以外部文学/历史掌故为主。",
      "蒋批款停发、晏子、李广、司马迁等偏历史/政治掌故，校对轮继续收紧。",
      "李敖自身事件和《二李通信》中的冤狱案情材料已排除。",
      "故事正文未改写，均按源文原句截取。"
    ],
    proofreadNotes: [
      "校对轮删除 3 条，保留 9 条。",
      "删除晏子不死君难：原文重在君臣类型阐释，故事动作没有展开。",
      "删除杨朴背断送老头皮诗：原文主要是李敖幼年见扇面和诗句考据，故事本体不足。",
      "删除李广杀霸陵尉后请罪：原文预设读者已知该典故，未交代前因后果，偏单句借典。",
      "保留蒋批款停发和司马迁两条：虽偏历史/政治掌故，但原文具备人物、动作、转折和明确用意。"
    ],
