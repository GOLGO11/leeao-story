...
const BOOK = "老贼臭史";
const SLUG = "laozei_choushi";
const ROUND = "story_round1";
const STATUS = "校对轮";
const ID_PREFIX = "LZCS";
const ORIGINAL_EXTRACTION_COUNT = 4;
const PROOFREAD_DROP_COUNT = 0;
const PROOFREAD_ADD_COUNT = 2;
...
const selections = [
  {
    prefix: "010",
    paragraph: 37,
    title: "盛世才送的“两箱财宝”原是马恩列斯著作"
  },
  {
    prefix: "010",
    paragraph: 55,
    title: "蒋介石设宴试探俞秀松遗孀是否愿意再婚",
    start: "有一次蒋介石和宋美龄发来请帖",
    end: "只是看我是否“回心转意”罢了。"
  },
  {
    prefix: "012",
    paragraph: 7,
    title: "手下假造公文给年老的谷正纲批",
    start: "他每天还照样去上班",
    end: "他的手下，乃假造大量公文给他批。"
  },
  {
    prefix: "013",
    paragraph: 2,
    title: "伏尔泰一句话化解英国人的围攻"
  },
  {
    prefix: "013",
    paragraph: 8,
    title: "宋高宗逃过追兵后赐甜菜名为“君踏”",
    start: "宁波有一种甜菜，叫“君踏”",
    end: "特以“君踏”赐名。"
  },
  {
...
function writeNotes(rows, validation, aggregate, manifest) {
  ...
  const lines = [
    ...
    `- 提取轮 ${manifest.originalExtractionCount} 条，校对轮保留 ${validation.count} 条；补入 ${manifest.proofreadAddCount} 条，未删除、未收短、未改题。`,
...
function main() {
  ...
  const manifest = {
    ...
    criteria:
      "校对轮只保留可脱离人物罪状、政治案件与史料证据链独立复述，有明确场景、行动、转折或结果，并用于揭示人物或说明道理的小故事；排除李敖自己的事件、案件经过、政治履历、秘密文件、薄事实、纯评论和总库已有同核故事。",
    excludedByStandard,
    duplicateChecks: [
      "盛世才逼丁慰慈把受贿数字由五万报到一百万、再降为五十万的故事已在《李敖大哥大》收录，本轮不重复。",
      "谷正纲在南美分不清人地、当面骂王昇与梁永章等故事已在《我梦碎，所以我梦醒》收录，本轮不重复。",
      "谷正纲声称梦见蒋介石托话的故事已在《我梦碎，所以我梦醒》收录，本轮不重复。",
      "罗素遇见自称恺撒者的故事已在《李敖文存》收录，本轮不重复。"
    ],
    proofreadAdds: [
      {
        id: "LZCS005",
        title: "宋高宗逃过追兵后赐甜菜名为“君踏”",
        reason: "提取轮漏收的名称来历故事，有追杀、脱险、赐名和明确用途。"
      },
      {
        id: "LZCS006",
        title: "蒋彦士陷害徐柏园后登门哭诉告罪",
        reason: "带来源限定、前因后果和人物判断，可脱离履历独立成篇。"
      }
    ],
    proofreadDrops: [],
    proofreadTightened: [],
    ...
    proofreadNotes: [
      `提取轮 ${ORIGINAL_EXTRACTION_COUNT} 条，校对轮补入 ${PROOFREAD_ADD_COUNT} 条，最终保留 ${rows.length} 条。`,
      "原有四则均继续保留，未删除、未收短、未改题。",
      "补入宋高宗与君踏的来历故事，以及蒋彦士登门哭诉告罪的转述轶事。",
      "陶希圣、方治、梁肃戎、俞秀松等其余候选仍属政治证据链、战争事件、薄事实或人物履历。"
    ],
