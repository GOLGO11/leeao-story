...
const BOOK = "白眼看台湾";
const SLUG = "baiyan_kan_taiwan";
const ROUND = "story_round1";
const STATUS = "校对轮";
const ID_PREFIX = "BYKTW";
...
const selections = [
  {
    prefix: "003",
    paragraphs: [7, 8, 9, 10, 11, 12, 13, 14],
    title: "《明儒学案》中王锡爵与顾宪成互说怪事"
  },
  {
    prefix: "003",
    paragraph: 25,
  ...
  },
  {
    prefix: "005",
    paragraphs: [29, 30],
    title: "蔡元培说政府不理政纲就不再帮忙"
  },
  {
    prefix: "005",
    paragraphs: [47, 48],
    title: "邦斯尔记郝斯把格拉德斯通的话拿给威尔逊"
  },
  {
    prefix: "013",
    paragraphs: [39, 40, 41],
  ...
  },
  {
    prefix: "015",
    paragraph: 31,
    title: "邓维桢讲甘地让印度人不还手走向盐田封锁线",
    start: "甘地一向不主张暴力解决问题"
  },
  {
    prefix: "018",
    paragraphs: [2, 3],
    title: "小仲马用“家族起点”反讽问话者"
  }
];
...
function main() {
  ...
  const manifest = {
    ...
    criteria:
      "校对轮逐条复核故事闭合度、来源限定、原文边界与总库重复；只保留可脱离台湾党外路线争论、人物评论、声明与书信独立复述，有明确场景、行动、转折或结果，并用于揭示人物或说明道理的小故事；排除李敖自己的事件、第三方人物履历、政治与论战经过、材料链、薄事实、纯评论和总库已有同核故事。",
    excludedByStandard,
    duplicateChecks: [
      "王锡爵与顾宪成互说怪事在本书003与005两篇重复，只取003篇先出现且出处完整的一版。",
      "两则蝙蝠寓言已在《为历史拨云》以WLSBY029、WLSBY030收录，鸟兽间改换身份一则另见《李敖有话说》LAYH070，本轮不重复。",
      "教皇回答斯大林军队在天上一则已在《李敖议坛哀思录》以LAYT010收录，本轮不重复。",
      "林肯两匹马分两次报死已在《笑敖年代》《李敖语妙天下》《李敖演讲集》收录，本轮不重复。",
      "曾子临死换席已在《李敖书札集》《挑战李敖》《笑敖年代》收录，本轮不重复。",
      "林肯问格兰特喝什么酒、要送两箱一则已在《李敖语妙天下》以LAYM030收录，本轮不重复。",
      "狙公朝三暮四哄猴子已在《坐牢家爸爸给女儿的八十封信》《挑战李敖》《李敖有话说》收录；刘基改写的猴群觉醒反抗为不同故事，予以保留。"
    ],
    proofreadAdds: [],
    proofreadDrops: [],
    proofreadTightened: [
      {
        id: "BYKTW006",
        fromStart: "□甘地一向不主张暴力解决问题",
        toStart: "甘地一向不主张暴力解决问题",
        reason: "去掉访谈答话符号，正文从完整句首开始；故事内容未删。"
      }
    ],
    proofreadTitleAdjusted: [
      {
        id: "BYKTW006",
        from: "甘地让印度人不还手走向盐田封锁线",
        to: "邓维桢讲甘地让印度人不还手走向盐田封锁线",
        reason: "标题补入本段访谈的讲述者；正文内容未改。"
      }
    ],
    proofreadContextCompleted: [],
    proofreadNotes: [
      "提取轮原有七则全部保留，未新增或删除故事。",
      "BYKTW006 去掉访谈答话符号，并在标题中补入讲述者邓维桢。",
      "七则正文均为连续原文，没有改写、补句或跨越原文空缺拼接。",
      "严光、丘吉尔、胜保等材料仍按薄事实排除；林义雄母女遇害一段属于政治事件，不补入。",
      "本书内重复版本和总库已有同核故事继续排除。"
    ],
    extractionNotes: [
      `候选扫描覆盖 ${sourceFiles().length} 个正文文件，机器候选 ${candidateCount()} 条。`,
      `提取轮保留 ${ORIGINAL_EXTRACTION_COUNT} 条，正文均能回到连续原文。`,
      "保留能独立说明人物品格或道理的问答、寓言和历史轶事；原文中的转述与出处限定一并保留。",
      "党外人物的政治经历、论战、声明和书信材料不按故事拆分。",
