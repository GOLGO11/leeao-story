...
const BOOK = "李敖演讲集";
const SLUG = "li_ao_yanjiangji";
const ROUND = "story_round1";
const STATUS = "校对轮";
const ID_PREFIX = "LAYJ";
...
function findSourceRoot() {
  ...
  const bookDir = fs
    ...
    .find((entry) => entry.isDirectory() && entry.name.startsWith("011."));
  if (!bookDir) throw new Error("Missing 011.李敖演讲集 source directory");
...
function main() {
  ...
  const manifest = {
    ...
    extractionNotes: [
      ...
      `提取轮初选 ${ORIGINAL_EXTRACTION_COUNT} 条；校对删除 ${proofreadDrops.length} 条、补入 ${proofreadAdds.length} 条、修边 ${proofreadTrims.length} 条，保留 ${rows.length} 条。`,
      "正文均按源文原段或段内原文截取。",
      "本轮重点剔除跨书已收的同一故事，并补入总表未收的独立笑话、掌故。"
    ],
